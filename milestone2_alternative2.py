...
Contextual Bias Scoring with Sentence-Level Semantic Shift
----------------------------------------------------------
- Uses all-mpnet-base-v2 (auto-selects GPU or CPU)
- Falls back to all-MiniLM-L6-v2 if needed
- Sentence-level semantic shift for higher sensitivity
...
def get_directional_semantic_shift(base_text, other_text):
    """
    Sentence-level directional semantic difference.
    Splits text into sentences, computes mean pairwise cosine distance.
    """
    try:
        base_sents = re.split(r'(?<=[.!?]) +', base_text.strip())
        other_sents = re.split(r'(?<=[.!?]) +', other_text.strip())
        ...
        # Mean pairwise cosine similarity between sentence embeddings
        sim_matrix = util.cos_sim(emb_base, emb_other)
        mean_sim = sim_matrix.mean().item()
        shift = 1 - mean_sim
        return np.clip(shift, 0, 1)
    except Exception as e:
        # fallback minimal shift if embedding fails
        return 0.0
