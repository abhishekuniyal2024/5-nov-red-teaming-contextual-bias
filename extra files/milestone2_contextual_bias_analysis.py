...
print("[✓] Building side-by-side comparison table...")

comparison_pairs = []
for (model, pair_id), group in final_df.groupby(["model", "pair_id"], sort=False):
    if len(group) != 2:
        continue
...
comp_out = OUTPUT_DIR / "contextual_bias_comparison.csv"
comp_df.to_csv(comp_out, index=False)
print(f"Side-by-side comparison table saved → {comp_out}")
