...
Per-response contextual bias analysis (directional) for LLM outputs.
"""
...
def get_directional_semantic_shift(base_text, other_text):
    ...
    try:
        ...
        sim_self = util.cos_sim(emb_base, emb_base).item()  # =1 but kept for structure
        sim_cross = util.cos_sim(emb_base, emb_other).item()
        shift = (1 - sim_cross) / (1 - 0.5 * sim_self)
        return np.clip(shift, 0, 1)
    except Exception:
        return 0.0

def get_directional_adj_diff(base_text, other_text):
    """Fraction of adjectives in base_text that are NOT in other_text"""
...
def compute_contextual_bias(sentiment, semantic_shift, adj_diff):
    score = (0.4 * abs(sentiment)) + (0.3 * semantic_shift) + (0.3 * adj_diff)
...
for model_name in sorted(df["model"].unique()):
    ...
    for pair_id, group in model_data.groupby("pair_id", sort=False):
        ...
        # Directional semantic + adjective
        ...
        records.append({**row_a, "sentiment_score": sentiment_a, "sentiment_bias": bias_a,
                        "semantic_shift": semantic_a, "adj_diff": adj_a, "contextual_bias_score": contextual_a})
        records.append({**row_b, "sentiment_score": sentiment_b, "sentiment_bias": bias_b,
                        "semantic_shift": semantic_b, "adj_diff": adj_b, "contextual_bias_score": contextual_b})
...
# Normalize sentiment
final_df["sentiment_norm"] = (final_df["sentiment_score"] - final_df["sentiment_score"].min()) / (
    final_df["sentiment_score"].max() - final_df["sentiment_score"].min()
)
